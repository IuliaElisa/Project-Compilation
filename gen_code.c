...
char str[10];
int to_add=0; // ajouter vars temps (2eme etape)
...
void freeSymTable(){
    struct Symbol *ptr, *tmp;
    for(int i=0;i<Symbols_Table.nbSymbols;i++){
...
void freeAST(struct Ast_node *p){
         if (p != NULL) {
          freeAST(p->child_node[3]);
          freeAST(p->child_node[2]);
          freeAST(p->child_node[1]);
          freeAST(p->child_node[0]);
          free(p);
       }
}
...
void generateCode(struct Ast_node *p, int level){
    ...
    switch (p->node_type)
    {
        ...
        case astProgram:
            ...
            break;
       /* case astExtDecl:
            processExternalDeclaration(p, level);
            break;*/
        case astPointerTo:
...
void traverse(struct Ast_node *p, int n)
{
    ...
    if (p)
    {
        switch (p->node_type)
        {
            ...
            case astFunctionDecl:
                spacing(n);
                printf("astFunctionDecl\n");
                break;
            case astFunctionDef:
                spacing(n);
                printf("astFunctionDef\n");
                break;
            case astParamList:
                spacing(n);
                printf("astParamList\n");
            ...
            default:
                printf("%d\n",p->node_type);
                printf("Error in semantics.c:TRAVERSING: No such Node Type found\n");
                break;
        }
        for (i = 0; i < 4; i++)
